import json
import logging
from matching.llm_orchestrator import query_llm_securely

logger = logging.getLogger(__name__)

def evaluate_job_with_llm(job_description: str, resume_text: str) -> float:
    """
    Use LLM (Ollama or external API) via secure orchestrator to semantically score the job vs resume.
    ...
    try:
        response_text = query_llm_securely(prompt, json_format=True)
